...
class post_type_settings extends generals {
	public function show(): void {
        ...
        <nav>
            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                <button class="nav-link <?= ( ( $tab==='home' ) ? 'active' : '' );  ?>" id="nav-home-tab" data-bs-toggle="tab" data-bs-target="#nav-home" type="button"
                ...
                <button class="nav-link <?= ( ( $tab==='profile' ) ? 'active' : '' );  ?>" id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile" type="button"
                        role="tab" aria-controls="nav-profile" aria-selected="false">
					<?php echo esc_html__('Created Post Types', 'cpt_plugin'); ?>
                </button>
                <button class="nav-link <?= ( ( $tab==='code' ) ? 'active' : '' );  ?>" id="nav-code-tab" data-bs-toggle="tab" data-bs-target="#nav-code" type="button"
                        role="tab" aria-controls="nav-code" aria-selected="false">
		            <?php echo esc_html__('Extract post type codes', 'cpt_plugin'); ?>
                </button>
            </div>
        </nav>
        <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade <?= ( ( $tab==='home' ) ? 'show active' : '' );  ?>" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
				<?php $this->addPostType() ?>
            </div>
            <div class="tab-pane fade <?= ( ( $tab==='profile' ) ? 'show active' : '' );  ?>" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
				<?php $this->editPostType() ?>
            </div>

            <div class="tab-pane fade <?= ( ( $tab==='code' ) ? 'show active' : '' );  ?>" id="nav-code" role="tabpanel" aria-labelledby="nav-code-tab">
		        <?php $this->extractPostTypeCodes() ?>
            </div>
        </div>
	...
	private function editPostType()
	{
        ...
        <table class="table">
            ...
            <tr>
                ...
                <th scope="col">Get post type code</th>
            </tr>
            </thead>
            <tbody>
			<?php
			...
			foreach ($options as $option):
                ...
                <tr>
                    ...
                    <td><a type="button" class="btn btn-primary"
                           href="admin.php?page=cpt_manager&edit_post=<?= $option['post_type'] ?>">Edit</a></td>
                    <td>
                    ...
                    <td>
                        <form method="post" action="admin.php?page=cpt_manager&tab=code">
			                <?php
			                echo '<input type="hidden" name="post_type" value="'.$option['post_type'].'">';
			                submit_button(esc_html__('Get Codes', 'cpt_plugin'), 'btn btn-warning');
			                ?>
                        </form>
                    </td>
                </tr>
    ...
    private function extractPostTypeCodes()
    {
        if( isset( $_POST['post_type'] ) && trim( $_POST['post_type'] )!=='' ){
	        $ExpDetail                        = get_option( 'cpt_manager_plugin_settings' )[trim( $_POST['post_type'] )];
	        ...
	        $post_type = array(
		        ...
		        'hierarchical'          => isset($ExpDetail['hierarchical']) ? 'true' : 'false',
		        'public'                => isset($ExpDetail['public']) ? 'true' :'false',
		        'show_ui'               => isset($ExpDetail['show_ui']) ? 'true' :'false',
		        'show_in_menu'          => isset($ExpDetail['show_in_menu']) ? 'true' :'false',
		        'menu_position'         => intval($menu_pos),
		        'show_in_admin_bar'     => isset($ExpDetail['show_in_admin_bar']) ? 'true' :'false',
		        'show_in_nav_menus'     => isset($ExpDetail['show_in_nav_menus']) ? 'true' :'false',
		        'can_export'            => isset($ExpDetail['can_export']) ? 'true' :'false',
		        'has_archive'           => isset($ExpDetail['has_archive']) ? 'true' :'false',
		        'exclude_from_search'   => isset($ExpDetail['exclude_from_search']) ? 'true' :'false',
		        'publicly_queryable'    => isset($ExpDetail['publicly_queryable']) ? 'true' :'false',
...
<?php
            ...
            echo 'You need to choose a post type from second tab.';
            var_dump( get_term( 'مجله', 'category' ) );
        }
    }
}